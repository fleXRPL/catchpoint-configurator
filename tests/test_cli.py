...
def test_cli_with_invalid_credentials(runner):
    ...
    with patch.dict(
        ...
        result = runner.invoke(catchpoint)
        assert result.exit_code == 1
...
def test_cli_help(runner):
    ...
    result = runner.invoke(catchpoint, ["--help"])
    assert result.exit_code == 0
    assert "Catchpoint Configurator CLI" in result.output
...
def test_command_help(runner):
    ...
    commands = [deploy, validate, list_tests, delete, export, import_config]
...
def test_invalid_command(runner):
    ...
    result = runner.invoke(catchpoint, ["invalid"])
