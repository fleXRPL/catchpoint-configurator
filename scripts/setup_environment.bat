...
if "%CATCHPOINT_API_KEY%"=="" (
    echo %YELLOW%Catchpoint API key not found in environment.%NC%
    echo Please set up your credentials by running:
    echo setx CATCHPOINT_API_KEY "your-api-key"
)
if "%CATCHPOINT_APP_KEY%"=="" (
    echo %YELLOW%Catchpoint Application key not found in environment.%NC%
    echo setx CATCHPOINT_APP_KEY "your-application-key"
)

:: Create example configuration if it doesn't exist
if not exist config\catchpoint_config.yaml (
    echo %YELLOW%Creating example catchpoint configuration...%NC%
    (
        echo version: "1.0"
        echo dashboards:
        echo   - name: "Example Configuration"
        echo     description: "Example catchpoint configuration"
        echo     widgets:
...
echo.
echo Next steps:
echo 1. Set up your Catchpoint credentials if you haven't already
echo 2. Review and modify config\catchpoint_config.yaml
echo 3. Run 'catchpoint-configurator config\catchpoint_config.yaml' to deploy your first catchpoint
echo.
echo For more information, visit: https://github.com/fleXRPL/catchpoint-configurator/wiki/Getting-Started
