...
if [ -z "${CATCHPOINT_API_KEY}" ] || [ -z "${CATCHPOINT_APP_KEY}" ]; then
    echo -e "${YELLOW}DataDog credentials not found in environment.${NC}"
    echo -e "${YELLOW}Please set up your credentials by running:${NC}"
    echo -e "export CATCHPOINT_API_KEY='your-api-key'"
    echo -e "export CATCHPOINT_APP_KEY='your-application-key'"
    echo -e "\n${YELLOW}You can add these to your ~/.bashrc or ~/.zshrc for persistence${NC}"
fi

# Create example configuration if it doesn't exist
if [ ! -f "config/catchpoint_config.yaml" ]; then
    echo -e "${YELLOW}Creating example catchpoint configuration...${NC}"
    cat > config/catchpoint_config.yaml << EOL
version: "1.0"
dashboards:
  - name: "Example Configuration"
    description: "Example catchpoint configuration"
    widgets:
      - title: "CPU Usage"
        type: "timeseries"
...
echo -e "2. Review and modify config/catchpoint_config.yaml"
echo -e "3. Run 'catchpoint-configurator config/catchpoint_config.yaml' to deploy your first catchpoint"
echo -e "\nFor more information, visit: https://github.com/fleXRPL/catchpoint-configurator/wiki/Getting-Started" 